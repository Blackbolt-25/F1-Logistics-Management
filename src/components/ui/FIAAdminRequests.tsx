...
import UpdateDesignRequestDialog from './UpdateDesignRequestDialog'
...
export default function FIAAdminRequests() {
  const [requests, setRequests] = useState<Request[]>([]);
  const [fiaAdminId, setFIAAdminId] = useState<string | null>(null);
  ...
  const fetchFIAAdminId = async () => {
    const credentials = JSON.parse(sessionStorage.getItem('credentials') || '{}');
    const { username, password } = credentials;

    try {
      const response = await fetch('/api/user/fia-admin-id', {
        headers: {
      ...
      if (response.ok) {
        ...
        console.log(data.fiaAdminId)
        setFIAAdminId(data.fiaAdminId);
      } else {
        console.error('Failed to fetch FIA Admin ID');
      }
    } catch (error) {
      console.error('Error fetching FIA Admin ID:', error);
    }
  };

  const fetchRequests = async () => {
    if (!fiaAdminId) return;
    try {
      const credentials = JSON.parse(sessionStorage.getItem('credentials') || '{}');
      const { username, password } = credentials;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold">Design Approval Requests</h2>
      </div>
      <Table>
        <TableCaption>A list of requests for design approval</TableCaption>
        <TableHeader>
          <TableRow>
